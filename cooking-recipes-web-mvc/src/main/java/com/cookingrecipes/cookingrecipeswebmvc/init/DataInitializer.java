...
public class DataInitializer implements ApplicationRunner {
	...
	private final List<User> DEFAULT_USERS = List.of(new User("Admin", "admin", "Admin1!"), new User("User", "user", "User1!"));
	...
	public void run(ApplicationArguments args) throws Exception {
		if (userService.count() == 0) {
			log.info("Successfully created users: {}", DEFAULT_USERS.stream().map(userService::create).collect(Collectors.toList()));
		}
		...
		if (recipeService.count() == 0) {
			...
			if (users.size() > 0) {
				...
				log.info("Successfully created recipes: {}", DEFAULT_RECIPES.stream().peek(recipe -> recipe.setAuthorId(author.getId()))
